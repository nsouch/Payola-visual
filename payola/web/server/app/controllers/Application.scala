...
object Application extends PayolaController with Secured
{
    ...
    def authenticate = Action {implicit request =>
        loginForm.bindFromRequest.fold(
            ...
            user => Redirect(routes.Application.dashboard).withSession( request.session + ("email" -> user._1))
        )
    }
    ...
    def register = Action {implicit request =>
        signupForm.bindFromRequest.fold(
            ...
            {   try {
                    ...
                    Redirect(routes.Application.dashboard).withSession(request.session + ("email" -> user._1))
                } catch {
