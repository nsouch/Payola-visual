...
class MiscellaneousSpecs extends CompilerIndependentSpec
{
    "Miscellaneous" should "support conversion methods between numeric types" in {
        compileScalaCode(
            """
                object o {
                    val c: Byte = 1
    ...
    it should "support custom operators without overriding default operators" in {
        compileScalaCode(
            """
                    class A {
                        def +(x: A): A = new A
    ...
    it should "preserve field and method names of adapter objects" in {
        compileScalaCode(
            """
