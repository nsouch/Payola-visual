...
abstract class ClassDefCompiler(val packageDefCompiler: PackageDefCompiler, val classDef: Global#ClassDef)
{
        ...
        typeApply.fun match {
            case Select(qualifier, name) if name.toString.matches("(is|as)InstanceOf") => {
                typeApply.args.head.tpe match {
                    case tpe if tpe != null && tpe.typeSymbol != null => {
                        compileInstanceOf(tpe.typeSymbol, name.toString.take(2) == "is", qualifier)
                    }
    ...
    private def compileLabelDef(labelDef: Global#LabelDef) {
        labelDef.name match {
            ...
            case name if name.toString.startsWith("case") || name.toString.startsWith("matchEnd") => {
                // Pattern matching labels generated by Scala 2.10.7
                // These are complex control flow constructs that we handle by:
                // 1. Wrapping in an immediately-invoked function to isolate scope
                // 2. Compiling the body which typically contains if-then-else chains
                // 3. Any jumps to this label will be handled as returns from the function
                buffer += "(function() {\n"
                labelDef.rhs match {
                    case Block(stats, expr) => {
                        stats.foreach(compileAstStatement(_))
                        compileAstStatement(expr, hasReturnValue = true)
                    }
                    case tree => {
                        compileAstStatement(tree, hasReturnValue = true)
                    }
                }
                buffer += "})()"
            }
